 ...
 */
  ...
  /**
 * \brief an example handle adding pushed kv into store 处理server收到的来自worker的请求
 * 
 */
  template <typename Val>
  struct KVServerDefaultHandle
  {
    // server处理收到的来自worker的请求
        ...
        const KVMeta &req_meta, const KVPairs<Val> &req_data, KVServer<Val> *server)
    {
      ...
      for (size_t i = 0; i < n; ++i)
      {
        ...
        if (req_meta.push)
        { //push
          //server receives data from worker, then store data in local.
          store[key] += req_data.vals[i]; //此处的操作是将相同key的value相加
        }
        if (req_meta.pull)
        {
          //pull
          //server sends requested vals
